...
class PlayPauseBar @JvmOverloads constructor(
    ...
    private val array=context.obtainStyledAttributes(attrs,R.styleable.PlayPauseBar)
...
///////////////////////////////
    //获取外部圆形的大小 默认2dp
    private val outerCircleSize=array.getDimensionPixelSize(R.styleable.PlayPauseBar_outer_circle_size,
        changDipIntoFloat(4f).toInt()
    )
    //获取外部圆形的颜色 默认颜色是白色
    private val outerCircleColor=array.getColor(R.styleable.PlayPauseBar_outer_circle_color,resources.getColor(R.color.default_outer_circle_color))
    private val outerPaint=Paint()

////////////////////////////////
    //外部正在加载的时候的宽度
    private val loadingCircleSize=array.getDimensionPixelSize(R.styleable.PlayPauseBar_loading_circle_size,
        changDipIntoFloat(4f).toInt()
    )

    //外部加载时候的颜色
    private val loadingCircleColor=array.getColor(R.styleable.PlayPauseBar_loading_circle_color,resources.getColor(R.color.default_loading_circle_color))
    private val loadingPaint=Paint()


//////////////////////////////////
    //播放过程中bar的size 默认4dp
    private val playBarSize=array.getDimensionPixelSize(R.styleable.PlayPauseBar_play_bar_size,
        changDipIntoFloat(4f).toInt()
    )
    //播放过程中的bar的颜色 默认为白色
    private val playBarColor=array.getColor(R.styleable.PlayPauseBar_play_bar_color,resources.getColor(R.color.default_play_bar_color))
    private val playBarPaint=Paint()


    //暂停时候的颜色
    private val pauseIconColor=array.getColor(R.styleable.PlayPauseBar_pause_icon_color,resources.getColor(R.color.default_pause_icon_color))
    private val pausePaint=Paint()


    //播放键的颜色
    private val playIconColor=array.getColor(R.styleable.PlayPauseBar_play_icon_color, resources.getColor(R.color.default_play_icon_color))
